...
![CLI](https://img.shields.io/badge/Method-CLI-2E3440?style=for-the-badge&logo=gnubash&logoColor=white)
...
Semua langkah di atas bisa dilakukan lewat terminal. Cocok buat yang mau otomasi atau setup cepat tanpa klik-klik.

---

### Step 1 — Cek VG & Thin Pool yang Ada

```bash
# Cek Volume Group yang tersedia
vgs

# Cek Logical Volume (termasuk thin pool yang sudah ada)
lvs

# Cek detail thin pool di VG tertentu
lvs -a pve
```
...
```
  LV            VG  Attr       LSize
  data          pve twi-aotz-- 800.00g   ← ini thin pool yang sudah ada
...
### Step 2 — Hapus `local-lvm` dari Proxmox

```bash
# Hapus registrasi storage local-lvm dari Proxmox
pvesm remove local-lvm
```

> 💡 Perintah ini hanya menghapus dari konfigurasi Proxmox — data fisik di disk tidak ikut terhapus.
...
### Step 3 — Buat Thin Pool Baru di LVM (Kalau Belum Ada)

Kalau di VG kamu belum ada thin pool, buat dulu secara manual:

```bash
# Buat thin pool bernama "data" di VG "pve" sebesar 800GB
lvcreate -L 800G --thinpool data pve

# Atau pakai semua sisa space yang ada
lvcreate -l 100%FREE --thinpool data pve
```

> ⚠️ Kalau thin pool sudah ada (terlihat dari output `lvs`), skip step ini — langsung ke Step 4.

---

### Step 4 — Daftarkan LVM-Thin Pool ke Proxmox

```bash
# Daftarkan pool pertama: OwnerData
pvesm add lvmthin OwnerData \
  --vgname pve \
  --thinpool data \
  --content images,rootdir

# Daftarkan pool kedua: PVEPool (thin pool berbeda, atau bisa sama)
pvesm add lvmthin PVEPool \
  --vgname pve \
  --thinpool data \
  --content images,rootdir
```
...
|---|---|
| `lvmthin` | Type storage yang didaftarkan |
| `OwnerData` | Nama storage di Proxmox (ID) |
| `--vgname` | Nama Volume Group |
| `--thinpool` | Nama thin pool di dalam VG |
| `--content images,rootdir` | `images` = Disk image VM, `rootdir` = Container |

---

### Step 5 — Buat Directory Pool (Opsional)

```bash
# Buat direktori kalau belum ada
mkdir -p /var/lib/vz

# Daftarkan sebagai storage Directory di Proxmox
pvesm add dir PVEDataPool \
  --path /var/lib/vz \
  --content backup,iso,snippets,vztmpl,import
```

**Penjelasan nilai `--content`:**

| Nilai | Fungsi |
|---|---|
| `backup` | File backup VM/CT (.vma, .tar.zst) |
| `iso` | ISO image untuk instalasi OS |
| `snippets` | Script hook, cloud-init config |
| `vztmpl` | Container template (.tar.gz) |
| `import` | Import disk dari file |

---

### Step 6 — Disable `local`

```bash
# Disable storage local (tidak bisa dihapus, cukup dinonaktifkan)
pvesm set local --disable 1
```

Aktifkan lagi kalau perlu:
```bash
...
### Ringkasan Semua Command Sekaligus

```bash
# ── Hapus local-lvm ──────────────────────────────────────────
pvesm remove local-lvm

# ── Buat thin pool di LVM (kalau belum ada) ──────────────────
lvcreate -L 800G --thinpool data pve

# ── Daftarkan LVM-Thin pool ke Proxmox ───────────────────────
pvesm add lvmthin OwnerData \
  --vgname pve \
  --thinpool data \
  --content images,rootdir
...
```bash
# Lihat semua storage yang terdaftar dan statusnya
pvesm status

# Lihat konfigurasi storage di file config Proxmox
cat /etc/pve/storage.cfg

# Cek LVM thin pool di disk
lvs -a pve
```

Output `pvesm status` yang diharapkan:
```
Name          Type     Status           Total            Used       Available
OwnerData     lvmthin  active       838860800               0       838860800
PVEDataPool   dir      active       500000000         1000000       499000000
PVEPool       lvmthin  active       838860800               0       838860800
local         dir      inactive             0               0               0
```
